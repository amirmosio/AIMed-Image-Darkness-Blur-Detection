...
class WavletTransform(FeatureValue):
    def __init__(self, img):
        super().__init__(img, "wavlet transform")

    def wavlet_transform(self, img, threshold=35):
        ...
        if (np.sum(RGstructure) + np.sum(RSstructure)) == 0:
            ...
            BlurExtent = np.sum(BlurC) / (np.sum(RGstructure) + np.sum(RSstructure) + 0.0001)
        if np.isnan(Per) or np.isinf(Per):
            return 99999, BlurExtent

        return Per, BlurExtent

    def calculate_value(self):
        return self.wavlet_transform(self.img)[0]
